...
function InstallLammps() {
  DIR_DEST=/c/cygwin_wm/usr/local/lammps-30Jul16_32bit
...
#      even though it is working properly.
  ...
  cp /mingw32/bin/libstdc++-6.dll    ${DIR_DEST}/bin/        || exit 1
  cp /mingw32/bin/libgcc_s_dw2-1.dll ${DIR_DEST}/bin/        || exit 1
...
function InstallQE() {
  DIR_DEST=/c/cygwin_wm/usr/local/q-e-qe-6.4.1_32bit
  ...
  cp /mingw32/bin/libgcc_s_dw2-1.dll  ${DIR_DEST}/bin/ || exit 1
  cp /mingw32/bin/libgfortran-5.dll   ${DIR_DEST}/bin/ || exit 1
  cp /mingw32/bin/libquadmath-0.dll   ${DIR_DEST}/bin/ || exit 1
  cp /mingw32/bin/libwinpthread-1.dll ${DIR_DEST}/bin/ || exit 1
  cp /mingw32/bin/libgomp-1.dll       ${DIR_DEST}/bin/ || exit 1
  cp /c/Program\ Files\ \(x86\)/IntelSWTools/compilers_and_libraries/windows/redist/ia32_win/mkl/*.dll               ${DIR_DEST}/bin/ || exit 1
  cp /c/Program\ Files\ \(x86\)/IntelSWTools/compilers_and_libraries/windows/redist/ia32_win/compiler/libiomp5md.dll ${DIR_DEST}/bin/ || exit 1
  cp ${DIR_MSMPI}/mpiexec.exe        ${DIR_DEST}/bin/        || exit 1
  cp ${DIR_MSMPI}/msmpi.dll          ${DIR_DEST}/bin/        || exit 1
  cp ${DIR_MSMPI}/smpd.exe           ${DIR_DEST}/bin/        || exit 1
  cp ${DIR_MSMPI}/MicrosoftMPI-Redistributable-EULA.rtf ${DIR_DEST}/bin/        || exit 1
